#!/usr/bin/env python3
"""Standalone PyQt6 operator UI for the v5 cap-inspection runtime.
...
from cap_line_v5.config import GPIO_BACKENDS, RuntimeConfig, normalize_gpio_backend, normalize_pixel_format
from cap_line_v5.runtime import resolve_pin_factory, run_detection
from cap_line_v5.types import RuntimeCallbacks


SCRIPT_DIR = Path(__file__).resolve().parent
DEFAULT_SETTINGS_PATH = str(SCRIPT_DIR / "cap_line_ui_v5_settings.json")
REJECT_LIMIT = 100
LIVE_POLL_INTERVAL_MS = 16
TRIGGER_PIN_LABEL = "Trigger Pin (BCM GPIO, e.g. 17)"
...
class HistoryRepository:
    ...
    def _initialize(self) -> None:
        with self._connection:
            self._connection.execute(
                """
                CREATE TABLE IF NOT EXISTS cap_line_history_v5 (
            ...
            self._connection.execute(
                "CREATE INDEX IF NOT EXISTS idx_cap_line_history_v5_recorded_at "
                "ON cap_line_history_v5 (recorded_at DESC)"
            )
    ...
    def upsert_record(self, record) -> None:
        with self._connection:
            self._connection.execute(
                """
                INSERT INTO cap_line_history_v5 (
...
class DetectionAppController:
    ...
    def start(self) -> bool:
        ...
        self.worker_thread = threading.Thread(
            target=self._worker_main, args=(config,), name="cap-line-v5-ui-worker", daemon=True
        )
...
if PYQT_AVAILABLE:
    ...
    class DetectionApp(QWidget):
        def __init__(
            ...
            *,
            ...
            self.setWindowTitle("Cap Line Inspector v5")
        ...
        def _build_ui(self) -> None:
            ...
            header = QHBoxLayout()
            title = QLabel("Cap Line Inspector v5")
        ...
        def _test_fire(self) -> None:
            ...
            threading.Thread(target=_worker, name="cap-line-v5-test-fire", daemon=True).start()
    ...
    class DetectionApp:  # type: ignore[no-redef]
        def __init__(self, *args, **kwargs):
            raise RuntimeError("PyQt6 is required to use cap_line_ui_v5.py")

    def main() -> None:
        raise RuntimeError("PyQt6 is required to run cap_line_ui_v5.py")
