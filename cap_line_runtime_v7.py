#!/usr/bin/env python3
"""Standalone v7 cap-inspection runtime entry point (headless).

Parse args -> build config -> run the two-stage detection loop until Ctrl-C.
Per-cap decisions are printed as they happen via a small history callback.
"""
...
from cap_line_v7.config import config_from_args, parse_args
from cap_line_v7.runtime import run_detection
from cap_line_v7.types import CapEventRecord, RuntimeCallbacks
...
def _print_cap_event(record: CapEventRecord) -> None:
    ...
    deduped = " (fire deduped)" if record.fire_suppressed else ""
    print(
