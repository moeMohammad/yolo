"""Slim v5 runtime configuration.

Same shape as v4's config with the v5 deltas from ``cap_line_v5_PROMPT.md``:
dirtv7 model, Raspberry Pi GPIO defaults (gpiozero, BCM GPIO17), and the
double-trigger fix knobs — ``merge_window_ms`` (renamed from v4's
``global_cooldown_ms``, now keyed to physical cap-exit times) plus the new
``min_fire_interval_ms`` post-fire refractory.
"""
...
from rpi_gpio_output import RPI_TRIGGER_PIN
...
# Must exceed ~2x the worst processed-frame interval (Pi 5 CPU inference is
# 50-100 ms/frame) or tracks fragment mid-cap; must stay below the gap between
# consecutive caps at one camera.
...
DEFAULT_GPIO_BACKEND = "rpi"
DEFAULT_TRIGGER_PIN = RPI_TRIGGER_PIN  # BCM GPIO17 = physical pin 11
DEFAULT_TRIGGER_DURATION = 0.3
DEFAULT_TRIGGER_MIN_GAP = 0.0
DEFAULT_LIVE_PREVIEW_FPS = 30.0
DEFAULT_DB_PATH = str(SCRIPT_DIR / "data" / "cap_line_history_v5.sqlite3")
...
class RuntimeConfig:
    ...
    def from_json_dict(cls, data: dict[str, Any]) -> "RuntimeConfig":
        ...
        Unknown keys are dropped so a v3/v4 settings file can be pointed at v5
...
def validate_config(config: RuntimeConfig) -> None:
    if len(config.cameras) != 2:
        raise ValueError("v5 requires exactly two cameras")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Run the standalone v5 cap inspection runtime (two cameras, one model, one nozzle)."
    )
