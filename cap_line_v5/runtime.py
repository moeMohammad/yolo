"""v5 runtime: one capture+inference loop per camera, one shared cap manager.
...
def resolve_pin_factory(config: RuntimeConfig):
    ...
    v5 targets a Raspberry Pi 5 (gpiozero); the Jetson driver from v1-v4 stays
    selectable via ``gpio_backend: "jetson"`` and is imported untouched from the
    original ``gpio_output.py``. ``simulate_gpio`` overrides both.
    """
...
class CameraWorker:
    ...
    def __init__(
        ...
        *,
        ...
        self.log_fn = log_fn
        self._sequence = 0
        self._thread = threading.Thread(target=self._run, name=f"cap-line-v5-camera-{self.camera_index}", daemon=True)
...
def run_detection(
    ...
    *,
    ...
    log_fn(f"Using v5 model: {model_path} imgsz={model_imgsz} target_fps={config.target_fps} gpio={scheduler.backend_name}")
    try:
