"""Cross-camera de-duplication and the once-per-cap fire guarantee (v5).
...
50 ms window. v5 keys everything off the physical exit timestamp
``track.last_seen`` instead, with three layers (see ``cap_line_v5_PROMPT.md``):
