#!/usr/bin/env python3
"""Standalone PyQt6 operator UI for the v6 cap-inspection runtime.
...
from cap_line_v6.config import GPIO_BACKENDS, RuntimeConfig, normalize_gpio_backend, normalize_pixel_format
from cap_line_v6.runtime import resolve_pin_factory, run_detection
...
class HistoryRepository:
    ...
    def upsert_record(self, record) -> None:
        with self._connection:
            self._connection.execute(
                """
                INSERT INTO cap_line_history_v6 (
                    event_id, recorded_at, result, class_name, confidence,
...
class DetectionAppController:
    ...
    def start(self) -> bool:
        ...
        self.worker_thread = threading.Thread(
            target=self._worker_main, args=(config,), name="cap-line-v6-ui-worker", daemon=True
        )
...
if PYQT_AVAILABLE:
    ...
    class DetectionApp(QWidget):
        ...
        def _build_ui(self) -> None:
            ...
            header = QHBoxLayout()
            title = QLabel("Cap Line Inspector v6")
    ...
    class DetectionApp:  # type: ignore[no-redef]
        def __init__(self, *args, **kwargs):
            raise RuntimeError("PyQt6 is required to use cap_line_ui_v6.py")

    def main() -> None:
        raise RuntimeError("PyQt6 is required to run cap_line_ui_v6.py")
